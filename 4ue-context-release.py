...
##################################################################################
    #check pdu release 
    check_rel=upf_check(child_upf,cmd='show upf stats pfcp msg | include SESS_MOD_REQ')
    gnb_cli(child_gnb=child_gnb,cli='uerl all')
    logging.info(gnb_show(child_gnb=child_gnb,cli='show stat'))
    check_rel1=upf_check(child_upf,cmd='show upf stats pfcp msg | include SESS_MOD_RSP')
    if str.isnumeric(check_rel) and str.isnumeric(check_rel1):
        if int(check_rel1) == int(check_rel) +1:
            logging.info('ue initiated context release request successful!')
        else:
            logging.error('ue initiated context release request failure!')
    else:
